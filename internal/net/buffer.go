...
// NOTE: this package is an adaption of pion/transport/packetio. If possible,
// the updates made in this repository will be reflected back upstream. If not,
// it is likely that this will be moved to a public package in this repository.
package net
...
func (b *PacketBuffer) WriteTo(p []byte, addr net.Addr) (int, error) {
	...
	if b.full {
		...
		// If write pointer is behind (wrapped around) or even with read
		// pointer, must move read pointer forward.
		b.full = false
		if b.write <= b.read {
			b.read += newPackets
		}
