...
func TestBufferAlloc(t *testing.T) {
	...
	// NOTE: these are noticeably higher than packetio.Buffer as each packet's
	// bytes.Buffer will allocate at least once.
	t.Run("100 writes", test(w, 100, 127))
	t.Run("200 writes", test(w, 200, 232))
	t.Run("400 writes", test(w, 400, 442))
	t.Run("1000 writes", test(w, 1000, 1051))
